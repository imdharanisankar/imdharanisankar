...
import re
import UserInfo_0811 as userinfo
import chat1
...
class ChatBotGUI(object):
    ...
    def __init__(self,base):
        ...
        self.sub_menu.add_command(label='Quit', command = base.destroy)
        ...
        self.chatpane = Text(base,bd=1, bg='light blue',state='disabled',width=47,height=22,\
                             wrap='word',spacing3='1',highlightbackground = 'black',highlightthickness = 1)
        self.chatpane.place(x=5, y=5, height=350, width=370)

        self.messagebox = Text(base,bd=1, bg='light grey',wrap='word',highlightbackground = 'black',\
                               highlightthickness = 1)
        self.messagebox.place(x=5, y=370, height=100, width=290)

        self.sendbutton = Button(base,text = 'Send', bd=1, bg='grey', activebackground = 'white', \
                                 font=('arial', 12, 'bold'), foreground = 'blue',relief = 'raised', \
    ...
    def DisplayData(self):
        ...
        if result == True:
            ...
            self.chatpane.insert(END,"\n"+self.UserText.format(self.maininput, self.date)+ self.i_p+"\n","right_Justify")
            #self.chatpane.tag_add("right_Justify",END)
            #self.chatpane.pack()
            #ResponseProcessing(chat1.input_process(self.i_p))
    ...
    def ResponseProcessing(self,response):
        ...
        self.chatpane.config(state="normal")
        self.chatpane.insert(END,self.UserText.format("CS ChatBot", self.date)+ response + "\n\n","left_Justify")
